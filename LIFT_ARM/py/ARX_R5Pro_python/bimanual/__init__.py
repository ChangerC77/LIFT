...
def find_first_specific_so_file(root_dir):
    for dirpath, dirnames, filenames in os.walk(root_dir):
        for filename in filenames:
            ...
            if filename.startswith('arx_r5pro_python') and filename.endswith('.so'):
                # 返回第一个找到的符合条件的文件路径
                return os.path.join(dirpath, filename)
    return None  # 如果没有找到符合条件的文件，则返回 None

# 使用 os.path.join 来拼接路径
so_file = find_first_specific_so_file(os.path.join(current_dir, 'api', 'arx_r5pro_python'))
...
__all__ = ['BimanualArm', 'SingleArm', 'arx_r5pro_python']
